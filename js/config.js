...
function toggleDevMode() {
  ...
  alert((!current
    ? '✅ Developer Mode ENABLED\nF12 and DevTools now accessible.'
...
var MASTER_PASSWORD_KEY = 'masterPassword';
var DEFAULT_MASTER_PASSWORD = 'admin123';
var TEMP_PASSWORD_KEY = 'tempPassword';
var TEMP_PASSWORD_EXPIRY_KEY = 'tempPasswordExpiry';

var TIME_SYNC_INTERNET_KEY = 'timeSync_internet';
var TIME_SYNC_DEVICE_KEY = 'timeSync_device';
var TIMER_VISIBLE_KEY = 'timerVisible';

var CUSTOMIZE_KEY = 'water_customize_v2';
var IMAGES_KEY = 'water_images_v2';
var SAVED_BGS_KEY = 'water_saved_bgs';
...
var CLOCK_DRIFT_TOLERANCE = 2 * 60 * 1000;
...
function getData(k) {
  try {
    return JSON.parse(localStorage.getItem(STORAGE[k]) || '[]');
  } catch (e) {
    return [];
  }
}

function saveDataLocal(k, d) {
  localStorage.setItem(STORAGE[k], JSON.stringify(d));
}
...
function getSettings() {
  try {
    var s = JSON.parse(localStorage.getItem(STORAGE.settings));
...
function saveCustomizationData(d) {
  localStorage.setItem(CUSTOMIZE_KEY, JSON.stringify(d));
  if (typeof scheduleSync === 'function') scheduleSync();
}
...
function saveImages(d) {
  localStorage.setItem(IMAGES_KEY, JSON.stringify(d));
}

function getSavedBgs() {
  try {
    return JSON.parse(localStorage.getItem(SAVED_BGS_KEY)) || [];
  } catch (e) {
    return [];
  }
}

function saveSavedBgs(a) {
  localStorage.setItem(SAVED_BGS_KEY, JSON.stringify(a));
}
...
function formatMoney(a) {
  ...
  if (s.currency === 'PHP') currencyCode = 'PHP';
  else if (s.currency === 'EUR') currencyCode = 'EUR';
...
function getCurrencyFormatter() {
  return { format: function (v) { return formatMoney(v); } };
}

function getCurrencySymbol() {
  var s = getSettings();
  if (s.currency === 'PHP') return '₱';
...
function setupAutoCapitalize() {
  ...
  document.body.addEventListener('blur', function (e) {
    ...
    // Skip passwords, emails, search boxes
    if (el.type === 'password' || el.type === 'email') return;
    if (el.classList.contains('search-box')) return;
    if (el.id === 'adminPassword' || el.id === 'masterPassword' || el.id === 'loginPassword') return;
    if (el.classList.contains('no-capitalize')) return;
...
function resizeImage(f, mW, mH, q) {
  ...
  if (f.size > MAX_IMAGE_SIZE_MB * 1024 * 1024) {
    return Promise.reject(new Error('File too large. Maximum ' + MAX_IMAGE_SIZE_MB + 'MB allowed.'));
  }
    ...
    r.onload = function (e) {
      ...
      i.onload = function () {
        ...
        try {
          ...
          rej(new Error('Failed to process image'));
        }
      };
