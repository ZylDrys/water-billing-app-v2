...
// Customers see their own submissions and can delete their copy.
// When they submit, a separate permanent copy is sent to the dev inbox.
// Even if they delete their copy, the dev copy remains until dev deletes it.
// ============================================
...
function submitSuggestion() {
    ...
    // === Save dev copy (permanent — only dev can delete from their inbox) ===
...
function deleteSuggestion(id) {
    if (!confirm('Delete your copy of this suggestion?')) return;
    // Only delete from user copy — dev copy is untouched
...
function renderSuggestionHistory() {
    ...
    el.innerHTML = countText + suggestions.slice(0, 15).map(function (s) {
        ...
        return '<div style="padding:6px;margin:3px 0;background:#f8f9fa;border-radius:4px;font-size:11px">' +
            ...
            '<button onclick="deleteSuggestion(' + s.id + ')" style="width:auto;padding:2px 6px;font-size:9px;background:var(--bg-danger);margin:0;min-width:20px" title="Delete your copy">✕</button>' +
            '</div></div>' +
            replyHtml +
            '</div>';
    }).join('') +
...
// === DEV SUGGESTION INBOX (master password area only) ===
...
function replyToDevSuggestion(id) {
    showPromptModal('Your reply to this suggestion:', false).then(function (reply) {
        if (!reply) return;
        var devSuggestions = JSON.parse(localStorage.getItem(DEV_SUGGESTIONS_KEY) || '[]');
        var entry = devSuggestions.find(function (s) { return s.id === id; });
        if (entry) {
            ...
            if (typeof scheduleSync === 'function') scheduleSync();
            renderDevSuggestionInbox();
            alert('✅ Reply saved! The user will see it in their suggestion history.');
        }
    });
}

function deleteDevSuggestion(id) {
    if (!confirm('Delete this suggestion from your inbox?\n(This only removes your dev copy.)')) return;
    var devSuggestions = JSON.parse(localStorage.getItem(DEV_SUGGESTIONS_KEY) || '[]');
...
// Dev uploads multiple QR codes (GCash, GoTyme, etc.)
// Users see a payment flow: contract → select method → QR
// ============================================
...
function addDevQrCode() {
    ...
    if (!name) { alert('Enter payment method name (e.g. GCash, GoTyme)'); nameEl.focus(); return; }
    ...
    resizeImage(f, 500, 500, 0.9).then(function (d) {
        ...
        alert('✅ QR code for "' + name + '" added!');
    }).catch(function (err) {
        alert('❌ Error uploading QR: ' + (err.message || 'Upload failed'));
    });
}
...
function renderDevQrCodesList() {
    ...
    el.innerHTML = codes.map(function (c) {
        return '<div style="padding:10px;border:1px solid var(--bg-card-border);border-radius:8px;margin:6px 0;display:flex;align-items:center;gap:10px">' +
            ...
            '</div>' +
            '<button onclick="deleteDevQrCode(' + c.id + ')" style="width:auto;padding:4px 8px;font-size:10px;background:var(--bg-danger);margin:0" title="Remove">✕</button>' +
            '</div>';
    }).join('') +
...
// === QR SECTION LOADER (master password area) ===
...
// === DEV PAYMENT FLOW (user-facing: contract → QR) ===
...
function showDevPaymentQr() {
    ...
    if (codes.length === 1) {
        ...
        selectorDiv.innerHTML =
            '<div style="font-size:11px;color:var(--text-muted);margin-bottom:6px">Select payment method:</div>' +
            '<div style="display:flex;gap:6px;flex-wrap:wrap;justify-content:center">' +
            codes.map(function (c, i) {
                return '<button onclick="selectDevPaymentQr(' + i + ')" ' +
                    'id="devQrBtn' + i + '" ' +
                    'style="width:auto;padding:6px 12px;font-size:12px;margin:0;background:' +
                    (i === 0 ? 'var(--bg-accent)' : 'var(--bg-table-even)') + ';color:' +
                    (i === 0 ? '#fff' : 'var(--text-secondary)') + '">' +
                    c.name + '</button>';
            }).join('') +
            '</div>';
        imgDiv.innerHTML = '<img src="' + codes[0].image + '" style="max-width:100%;max-height:250px;border-radius:8px">';
